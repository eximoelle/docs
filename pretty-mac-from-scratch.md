...
```
После выполнения скрипта нужно выполнить это для того, чтобы прописать Homebrew в PATH. Иначе система не будет знать, где находится `brew`.
```
(echo; echo 'eval "$(/opt/homebrew/bin/brew shellenv)"') >> /Users/maxlion/.zprofile
